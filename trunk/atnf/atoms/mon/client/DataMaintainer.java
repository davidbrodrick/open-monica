 ...
 * Description: handles client-side collection and buffering of Points
 * @author Le Cuong Nguyen
 **/

public class DataMaintainer
implements Runnable, PointListener
{
...
/*   public void addPointMonitor(Vector points)
   {
      for (int i = 0; i < points.size(); i++)
         if (points.elementAt(i) instanceof PointMonitor)
            addPointMonitor((PointMonitor)points.elementAt(i), true);   
   }
   
   public void addPointMonitor(ArrayList points)
   {
      for (int i = 0; i < points.size(); i++)
...
*/
   ...
   public static void subscribe(Vector points, PointListener pl)
   {
     ...
     if (realarg.size()>0) {
       ...
       long toffset = (MonClientUtil.getServer()).getCurrentTime()
	 ...
	 for (int i=0; i<res.size(); i++) {
	   ...
	   ArrayList al = PointInteraction.parseLine(pm, false);
	   Object[] obj = al.toArray();
	   for (int j=0; j<obj.length; j++) {
	     String[] names = ((FakeMonitor)obj[j]).getAllNames();
	     String source = ((FakeMonitor)obj[j]).getSource();
	     for (int k=0; k<names.length; k++) itsNames.put(source+"."+names[k], obj[j]);
     ...
     for (int i=0; i<points.size(); i++) {
       FakeMonitor fm = (FakeMonitor)itsNames.get((String)points.get(i));
       if (fm==null) continue;
   ...
   public static void subscribe(String point, PointListener pl)
   {
	...
	for (int i = 0; i < obj.length; i++) {
	  ...
	  for (int k = 0; k < names.length; k++) itsNames.put(source+"."+names[k], obj[i]);
          itsMain.addPointMonitor((FakeMonitor)obj[i], true);
          ((FakeMonitor)obj[i]).setTimeOffset((MonClientUtil.getServer()).getCurrentTime()
					      - AbsTime.factory().getValue());
	}
      }
   ...
   public static void unsubscribe(String pointname, String source, PointListener pl)
   {
   ...
   {
     if (itsNames.get(point) == null) return;
     FakeMonitor fm = (FakeMonitor)MonitorMap.getPointMonitor(point);
     fm.removePointListener(pl);
     itsBuffer.remove(point);
     if (fm.getNumListeners() < 2) {
       itsMain.removePointMonitor(fm);
       //Remove each name for the point
       String[] names = fm.getAllNames();
       itsNames.remove(point);
       //for (int i=0; i<names.length; i++)
       //itsNames.remove(source+"."+names[i]);
       //Next, let the collection thread know our points have changed
       //synchronized (itsMain.itsPoints) {
       //  itsMain.itsPoints.notifyAll();
	 //itsMain.interrupt();
       //}
     }
   ...
   public void run()
   {
	 ...
	 try {
	   ...
	   while (itsPoints.size() == 0) itsPoints.wait();
	 } catch (Exception e) {}
	 ...
	 now += 50000; // Fudge factor
	   ...
	   else break; //No more points awaiting collection
	 }
	 if (pm != null) {

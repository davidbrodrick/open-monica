 ...
 * configured to display various monitor points, to display data in a
 * static archival mode or a dynamic updating mode, and also has other
 * features.
 *
...
implements ActionListener, Runnable
{
  ...
  implements ActionListener
  {
    ...
    implements ActionListener
    {
      ...
      public
      void
      showSetup(SavedSetup setup, int number)
      {
	try {
	  ...
	  if (temp.equals("auto")) {
	    ...
	    if (temp.equals("true")) itsAutoZero.setSelected(true);
	    else if (temp.equals("false")) itsAutoZero.setSelected(false);
      ...
      public
      void
      getSetup(SavedSetup setup, int number)
      throws Exception
      {
	...
	if (itsAutoScale.isSelected()) {
	  ...
	  if (itsAutoZero.isSelected()) setup.put("showzero"+number, "true");
	  else setup.put("showzero"+number, "false");
	} else {
	  setup.put("scalemode"+number, "fixed");
	  //Check that the numeric fields contain parsable number
	  try {
	    double foo = Double.parseDouble(itsScaleMax.getText());
            foo = Double.parseDouble(itsScaleMin.getText());
	  } catch (Exception e) {
	    JOptionPane.showMessageDialog(this,
	...
	if (itsDrawLines.isSelected()) {
	  ...
	  else setup.put("gaps"+number, "false");
	...
	if (selpoints.size()==0) {
	  JOptionPane.showMessageDialog(this,
					...
					"at least one point for each axis!",
					"No Points Selected!",
					JOptionPane.WARNING_MESSAGE);
	  throw new Exception();
	}
	...
	if (itsAxis.size()==1) Toolkit.getDefaultToolkit().beep();
	else {
	...
	return null;
      ...
      if (temp==null || temp.equals("")) temp = "Data vs. Time";
...
//    }
  ...
  /** Merge new data into the data sets used by the graphing widget.
   * A few of the activities need to be performed by calling the GUI
   * event thread to prevent the data sets from becoming corrupted. */
  private
  void
  mergeData(int axis, int series, Vector data)
  {
    if (data==null) data = new Vector(); //Create dummy data
	...
	//Data is too old to keep
      ...
      } else break; //No more data needs to be removed
    }
	...
	//System.err.println("ProviderATTimeSeries:mergeData: Bad Data Point");
      }
    }
    ...
    if (data.size()>0) {
      ...
      ((Vector)itsPointEpochs.get(axis)).set(series,
					     ((PointData)data.lastElement()).getTimestamp());
    }
  }
  ...
  actionPerformed(ActionEvent e)
  {
    System.err.println("ATTimeSeries: Time to update graph \""
		       + itsTitle + "\"");
    synchronized (this) { this.notifyAll(); }
  }
  ...
  loadSetupReal()
  {
	...
	SwingUtilities.invokeLater(pleasewait);
      ...
      TimeZone tz = null;
      if (itsTimeZone!=null) tz = TimeZone.getTimeZone(itsTimeZone);
      for (int i=0; i<itsNumAxis; i++) {
	itsPointNames.add(new Vector());
	itsPointEpochs.add(new Vector());
	if (tz==null) itsData.add(new TimeSeriesCollection());
	else itsData.add(new TimeSeriesCollection(tz));
      }

      //Make new graph
      String tunits = "Time";
      if (itsPeriod.getValue()>3*86400000000l) tunits = "Date";
      if (tz!=null) tunits = tunits + " (" + tz.getDisplayName() + ")";
      final JFreeChart chart = ChartFactory.createTimeSeriesChart(itsTitle,
							    tunits, "Value",
							    (TimeSeriesCollection)itsData.get(0),
							    itsShowLegend,
							    true, false);
	...
	if (i==0) {
	  ...
	  plot.setSecondaryDataset(i-1, (TimeSeriesCollection)itsData.get(i));
	...
	if (temp==null) itsMaxSamps=0;
	else itsMaxSamps  = Integer.parseInt(temp);
	...
	if (temp.equals("auto")) {
	  ...
	  if (temp.equals("true")) newaxis.setAutoRangeIncludesZero(true);
	  else newaxis.setAutoRangeIncludesZero(false);
	} else if (temp.equals("fixed")) {
	  ...
	  System.err.println("ATTimeSeries:loadData: Unknown mode \"" +
			     temp + "\"");
	  blankSetup();
	  return false;
	}
	...
	if (temp.equals("lines")) {
	  ...
	  System.err.println("ATTimeSeries:loadData: Unknown style \"" +
			     temp + "\"");
	  blankSetup();
	  return false;
	}
	if (i==0) plot.setRenderer(renderer);
	else plot.setSecondaryRenderer(i-1, renderer);
	...
	for (int j=0; j<numpoints; j++) {
      ...
      /*DateAxis da2 = new DateAxis("foo");
       da2.setTickLabelsVisible(true);
       da2.setAutoRange(true);
       da2.setAutoTickUnitSelection(true);
  ...
  /** Convert the monitor 'PointData' to a JFreeChart 'TimeSeriesDataItem'.
   * This converts the timestamp to a suitable format and also converts
   * the data into a 'Number' type. If no conversion was possible, eg
   * because we cannot convert the data type into a Number, then <tt>null</tt>
   * will be returned.
  ...
  protected static
  TimeSeriesDataItem
  toTimeSeriesDataItem(PointData pd)
  {
    if (pd==null || pd.getTimestamp()==null || pd.getData()==null) return null;

    try {
      Millisecond t = new Millisecond(pd.getTimestamp().getAsDate());
      Number d = toNumber(pd.getData());
      //System.err.println(pd.getTimestamp().toString(AbsTime.Format.UTC_STRING) + "\t" + d);
      if (d==null) return null;
      return new TimeSeriesDataItem(t, d);
    } catch (Exception e) {
      //Some odd problems happen in the code called above...
      System.err.println("ATTimeSeries:toTimeSeriesDataItem: "
			 + e.getMessage());
      return null;
    }
  }
  ...
  export(PrintStream p)
  {
	...
	TimeSeries data = series.getSeries(c);
        if (data.getItemCount()==0) continue;
	//Print the header information for this series
	p.println("#" + data.getItemCount() + " data for "
		  + data.getName() + " follow");

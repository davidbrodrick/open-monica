...
public abstract class MonitorUtils {
  ...
  /**
   * Parse type code and multiple value strings and return the appropriate array of values.
   * 
   * @param type
   *          One of <tt>int</tt>, <tt>flt</tt>, <tt>dbl</tt>, <tt>str</tt>, <tt>bool</tt>.
   * @param strval
   *          An array of strings representations of data values, eg "3.141", or "true".
   * @return The appropriate array.
   * @throws IllegalArgumentException
   *           If the type code is invalid or the string value cannot be parsed.
   */
  public static Object parseFixedValueArray(String type, String[] strvals) throws IllegalArgumentException {
    Object[] res;
    try {
      if (type.equals("dbl")) {
        res = new Double[strvals.length];
      } else if (type.equals("flt")) {
        res = new Float[strvals.length];
      } else if (type.equals("int")) {
        res = new Integer[strvals.length];
      } else if (type.equals("str")) {
        res = new String[strvals.length];
      } else if (type.equals("bool")) {
        res = new Boolean[strvals.length];
      } else {
        throw new IllegalArgumentException("Unknown type code for value data type");
      }
      // Parse each value
      for (int i = 0; i < strvals.length; i++) {
        res[i] = parseFixedValue(type, strvals[i]);
      }
    } catch (Exception e) {
      throw new IllegalArgumentException("Unable to parse data value");
    }
  ...
  /*
   * public static String prunePointTree(String point, HashSet<String> selected, HashSet<String> allpoints) { String res = point;
   * int lastdot = point.lastIndexOf('.'); if (lastdot == -1) { System.err.println("no . in point name"); return point; } String
   * branch = point.substring(0, lastdot); System.err.println("for point " + point + " branch is " + branch); boolean allmatch =
   * true; Iterator<String> i = allpoints.iterator(); while (allmatch && i.hasNext()) { String thispoint = i.next(); if
   * (thispoint.startsWith(branch) && !selected.contains(thispoint)) { System.err.println("selected set doesn't contain " +
   * thispoint); allmatch = false; } else if (thispoint.startsWith(branch)) { System.err.println("selected set contains " +
   * thispoint); } } if (allmatch) { res = branch; } System.err.println("result is " + res); return res; }
   */

  /**
   * Method for pruning a set of points down to a minimal pattern with wildcards to indicate that all potential points from that
   * tree are selected. Use with {@link #sprout(HashSet, HashSet)} to return the pruned list to its original format
   * 
   * @param selections
   *          The Set of selected point names
   * @param allPoints
   *          The Set of all point names
  ...
  /**
   * Preparatory step to ensure that only points with a potential to be shortened are acted upon
   * 
   * @param selections
   *          The initial selected points
   * @param allPoints
   *          The all-encompassing total list of points
  ...
  private static HashSet<String> prunePointTreePrep(HashSet<String> selections, HashSet<String> allPoints) {
    ...
    for (String s : selected) {
      ...
      if (res.contains(branch + ".*"))
        continue; // go to next point s
   ...
   * 
   * @param prep
   *          The HashSet that has been "prepared" by the prunePointTreePrep() method
   * @param allPoints
   *          The HashSet of all points in their full form
   * @return A fully pruned HashSet
   */
  private static HashSet<String> prune(HashSet<String> prep, HashSet<String> allPoints) {
    HashSet<String> res = new HashSet<String>();
    ...
    for (String s : prep) {
      if (!s.endsWith(".*")) {
        ...
        if (doOvers.contains(branch + ".*") || res.contains(branch + ".*"))
          continue; // go to next point s
        boolean allmatch = true;
        Iterator<String> i = allPoints.iterator();
        while (allmatch && i.hasNext()) {
    ...
    if (doOvers.size() > 0)
      pruned = prune(doOvers, newAll);
    res.addAll(pruned);
    return res;
  }

  /**
   * Reversal method to retrieve the selected points from an already pruned point tree. Designed to be backwards-compatible with old
   * Setups of points that use a single list of many individual Strings.
   * 
   * @param prunedSet
   *          A HashSet of Strings that has been pruned already by {@link #prunePointTree(HashSet, HashSet)}
   * @param allPoints
   *          A HashSet of String containing a set of all possible defined points

...
/**
 * This Transaction checks if a set of points are all within their defined <i>limits</i>.
 * If the values for one or more points are not currently available then no output will be
 * generated unless there is an available point that is outside its limits in which case
 * we should clearly indicate an alarm condition even though some other points are
 * unavailable.
 * <P>
 * The behaviour of this translation is controlled by the arguments given to it in the
 * point definition: <br>
 ...
 * Some times a monitor point might have just one or two spurious readings so the second
 * argument specifies how many update cycles the listened-to points must be outside their
 * limits before we indicate a problem. <tt>eg, "3"</tt>. <br>
 * If the values are okay then the third argument string will be used for the output.
 * <tt>eg, this might be "OK"</tt>. <br>
 * If one or more of the values it outside their nominated limits for a sufficient number
 * of cycles then the fourth argument string will be used as output. <tt>eg, "ALARM"</tt>.
 * <br>
...
public class TransactionLimitCheck extends Transaction implements ActionListener
{
  ...
  /**
   * Holds the last few results of our analysis of the listened-to points. This is used
   * for when more than one update must indicate an out of limits state before we use the
   * alternate output string. The size is determined by the second init argument. An entry
   * of false indicates that there is no reason for alarm while true indicates an alarm
   * condition.
   */
  ...
  /**
   * Check each monitor point and return <tt>true</tt> if any are outside their limits,

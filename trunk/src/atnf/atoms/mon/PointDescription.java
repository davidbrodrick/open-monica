...
public class PointDescription implements ActionListener, NamedObject, Comparable {
    ...
    /**
     * A short description of this point for use by space limited client
     * programs.
     */
    ...
    /**
     * Return the period between updates for this monitor point. A period of
     * zero has the special meaning that the update frequency is being handled
    ...
    protected void makeTransactions()
    {
        ...
        for (int i = 0; i < itsInputTransactionStrings.length; i++) {
            inputtrans[i] = (Transaction) Factory.factory(this, itsInputTransactionStrings[i],
                    "atnf.atoms.mon.transaction.Transaction");
        }
        ...
        for (int i = 0; i < itsOutputTransactionStrings.length; i++) {
            outputtrans[i] = (Transaction) Factory.factory(this, itsOutputTransactionStrings[i],
                    "atnf.atoms.mon.transaction.Transaction");
        }
    ...
    protected void makeTranslations()
    {
        ...
        for (int i = 0; i < itsTranslationStrings.length; i++) {
            translations[i] = (Translation) Factory.factory(this, itsTranslationStrings[i],
                    "atnf.atoms.mon.translation.Translation");
        }
     ...
     * what real-world system the information contained in this point pertains
     * to. For instance this could indicate which antenna some monitor data was
     * collected from.
     */
    ...
    /**
     * Check the data value against the alarm criteria and set the alarm field
     * if the point is in an alarm state, otherwise leave the current alarm
     * value unchanged.
     */
    ...
    public static ArrayList<PointDescription> parseLine(String line) throws Exception
    {
        ...
        for (int i = 0; i < pointSourceArray.length; i++) {
            result.add(PointDescription.factory(pointNameArray, pointLongDesc, pointShortDesc, pointUnits, pointSourceArray[i],
                    pointInputArray, pointOutputArray, pointTranslateArray, pointLimitsArray, pointArchiveArray, notificationArray, 
                    pointPeriod, archiveLife, guidance, priority, pointEnabledArray[i]));
        }
    ...
    public static PointDescription factory(String[] names, String longdesc, String shortdesc, String units, String source,
            String[] inputs, String[] outputs, String[] translate, String[] limits, String[] archives, String[] notifications,
            String period, String archivelife, String guidance, String priority, boolean enabled)
    {
        ...
        for (int i = 0; i < listeners.length; i += 2) {
            if (listeners[i] == PointListener.class) {
                try {
                    ...
                    theirLogger.warn(getFullName() + ": Error distributing data to listener, class "
                            + listeners[i + 1].getClass().getCanonicalName() + " (" + e + ")");
                    e.printStackTrace();
                }
            }
        }
            ...
            if (data != null && itsTranslations != null) {
                for (int i = 0; i < itsTranslations.length; i++) {
                    try {
                        ...
                        theirLogger.error("(" + getFullName() + ") Error on Translation " + (i + 1) + "/" + itsTranslations.length
                                + ": " + e);
                        data = null;
                    }
                ...
                for (int i = 0; i < itsOutputTransactions.length; i++) {
                    ...
                    if (thistrans != null) {
                        ...
                        if (ds == null) {
                            ...
                            theirLogger.warn("(" + getFullName() + ") While writing output data: ExternalSystem "
                                    + thistrans.getChannel() + " is not connected");
                        } else {
                            try {
                                ds.putData(this, data);
                            } catch (Exception e) {
                                theirLogger.warn("(" + getFullName() + ") while writing output data, ExternalSystem "
                                        + ds.getName() + " threw exception \"" + e + "\"");
                            }
                        }
                    }
                }

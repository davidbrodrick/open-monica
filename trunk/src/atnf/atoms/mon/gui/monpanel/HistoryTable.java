 ...
 * 
 * <P>
 * When real-time mode is enabled this class uses the <i>DataMaintainer</i> to
 * stay current with any changes to the monitor point values.
 * 
 * <P>
 * The <i>PointData</i> objects for all monitor point updates are stored in the
 * field <i>itsData</i>. When it is time to update the table we call the method
 * <i>processData</i> which looks at what options were selected by the user and
 * organises <i>itsData</i> into appropriate table rows. The rows are stored in
 * the field <i>itsRows</i>, which is accessed whenever swing wants to redraw
 * any part of the table.
 * 
 * @author David Brodrick
 * @version $Id: HistoryTable.java,v 1.2 2006/06/27 05:51:03 bro764 Exp $
 */
public class HistoryTable extends MonPanel implements PointListener, Runnable,
		TableCellRenderer {
	static {
	...
	public class HistoryTableSetupPanel extends MonPanelSetupPanel {
		...
		protected JRadioButton itsStatic = new JRadioButton(
				"Archival Mode. Start Time (yyyy/MM/dd HH:mm):");
		...
		protected JRadioButton itsNewRow = new JRadioButton(
				"New row generated when ANY monitor point updates");
		/** Or new row when first point updates? */
		protected JRadioButton itsFirstRow = new JRadioButton(
				"New row generated when FIRST SELECTED monitor point updates");
		/** Or row for each update interval? */
		protected JRadioButton itsSharedRow = new JRadioButton(
				"New row generated every ");
		...
		protected JCheckBox itsChangesOnly = new JCheckBox(
				"Hide repeated rows (rows with no changes to data values)");
		/** Should we hide incomplete rows? */
		protected JCheckBox itsHideIncomplete = new JCheckBox(
				"Hide incomplete rows (rows where no data is available for one or more points)");
		/** Should a sparse table be shown? */
		protected JCheckBox itsSparse = new JCheckBox(
				"Sparse table (don't plot values in columns that didn't change)");
		...
		protected JCheckBox itsNoDate = new JCheckBox(
				"Suppress date in timestamps");
		/** Suppress the milliseconds in data timestamps? */
		protected JCheckBox itsNoMS = new JCheckBox(
				"Suppress milliseconds in timestamps");
		/** Should things be shown in chronological order? */
		protected JCheckBox itsReverse = new JCheckBox(
				"Chronological order (default is reverse chronological order)");
		...
		public HistoryTableSetupPanel(HistoryTable panel, JFrame frame) {
			...
			itsMainPanel
					.setLayout(new BoxLayout(itsMainPanel, BoxLayout.Y_AXIS));
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (e.getSource() == itsStart || e.getSource() == itsPeriod
			...
			if (cmd.equals("Dynamic")) {
				...
				System.err.println("HistoryTable:SetupPanel:actionPerformed: "
						+ cmd);
			}
		}

		/**
		 * Return the current setup, as determined by the GUI controls. It
		 * provides the means of extracting the setup specified by the user into
		 * a useable format.
		 * 
		...
		protected void showSetup(SavedSetup setup) {
			...
			if (setup == null) {
				System.err
						.println("HistoryTableSetupPanel:showSetup: Setup is NULL");
				return;
			}
			if (!setup.getClassName().equals(
					"atnf.atoms.mon.gui.monpanel.HistoryTable")) {
				System.err
						.println("HistoryTableSetupPanel:showSetup: Setup is for wrong class");
				return;
			}
		...
		while (itsKeepRunning) {
			if (itsReInit) {
				...
				Runnable clearTable = new Runnable() {
					public void run() {
						removeAll();
						add(new JLabel("Retrieving monitor data from server..",
								JLabel.CENTER));
						repaint();
					}
				};
	...
	public void blankSetup() {
		...
		Runnable clearTable = new Runnable() {
			public void run() {
				removeAll();
				add(new JLabel("Select display options using the tabs",
						JLabel.CENTER));
				repaint();
			}
		};
	 ...
	 * specify sub-class-specific information, so this method can be used to
	 * restore saved MonPanel states.
	 * 
	 * @param setup
	...
	public synchronized boolean loadSetup(final SavedSetup setup) {
		try {
			...
			if (itsFrame != null) {
				...
				System.err.println("HistoryTable:loadData: "
						+ e.getClass().getName());
			}
			blankSetup();
			return false;
	...
	/**
	 * Get the current sub-class-specific configuration for this MonPanel. This
	 * can be used to capture the current state of the MonPanel so that it can
	...
	public void onPointEvent(Object source, PointEvent evt) {
		...
		if (!foundit) {
			System.err.println("HistoryTable: got update for \"" + src
					+ "\" but not subscribed to it");
		} else {
			processData();
			Runnable updateTable = new Runnable() {
				public void run() {
	...
	/**
	 * Return the values for the next row based on time/data of previous row and
	 * user options.
	 */
	...
	public class HistoryTableModel extends AbstractTableModel {
		...
		public String getColumnName(int column) throws NullPointerException {
			...
			if (column == 0) {
				...
				String firstsrc = ((String) itsPoints.get(0)).substring(0,
						((String) itsPoints.get(0)).indexOf("."));
				for (int i = 1; i < itsPoints.size(); i++) {
					if (!firstsrc.equals(((String) itsPoints.get(i)).substring(
							0, ((String) itsPoints.get(i)).indexOf(".")))) {
						showsource = true;
						break;
					}
				}
				res = ((String) itsPoints.get(column - 1))
						.substring(((String) itsPoints.get(column - 1))
								.lastIndexOf(".") + 1);
				if (showsource) {
					String thissrc = ((String) itsPoints.get(column - 1))
							.substring(0, ((String) itsPoints.get(column - 1))
									.indexOf("."));
					res = res + " (" + thissrc + ")";
				}
			}
	...
	public Component getTableCellRendererComponent(JTable table, Object value,
		...
		/*
		 * PointDescription pm = getPoint(row, column); if (pm!=null) { //Limits
		 * are defined, get the raw data and check it PointData pd =
		 * DataMaintainer.getBuffer(pm.getSource() + "." + pm.getName()); if
		 * (pd!=null) { long age = (new AbsTime()).getValue() -
		 * pd.getTimestamp().getValue(); long period = pm.getPeriod(); if
		 * (period!=0 && age>2*period && age<5*period) { //The point is old, so
		 * alter the foreground color res.setForeground(Color.lightGray); }
		 * 
		 * PointLimit limits = pm.getLimits(); if (limits!=null) { if
		 * (pd.isValid() && (period==0 || age<5*period) &&
		 * !limits.checkLimits(pd)) { //Don't highlight the point if it has
		 * expired //Point is outside of limits, so highlight this cell if
		 * (age<3*period) res.setForeground(Color.red); else
		 * res.setForeground(Color.orange); if (res instanceof JComponent)
	...
	public static void main(String[] argv) {
		 ...
		 * );
		 * 
		 * HistoryTable pt = new HistoryTable(); pt.loadSetup(seemon); //
		 * frame.getContentPane().add(pt); frame.setContentPane(pt);
		 * 
		 * frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE); frame.pack();
		 * frame.setVisible(true);
		 */
		/*
		 * try { RelTime sleepy = RelTime.factory(15000000l); sleepy.sleep(); }

 ...
 * of one vs. the value of the other.
 * 
 * @author David Brodrick
 * @version $Id: ATXYPlot.java,v 1.7 2006/05/26 01:32:13 bro764 Exp $
 * @see MonPanel
 */
public class ATXYPlot extends MonPanel implements ActionListener, Runnable {
  static {
    MonPanel.registerMonPanel("X vs Y Plot", ATXYPlot.class);
  }

  public class ATXYPlotSetupPanel extends MonPanelSetupPanel
    ...
    protected JRadioButton itsDynamic = new JRadioButton(
        "Real-Time (Dynamic plot showing latest data)");
    ...
    public JRadioButton itsShowAllMatches = new JRadioButton(
        "Show all matched X/Y points");

    /** Radio button to enable display of post-processed data. */
    public JRadioButton itsPostProcess = new JRadioButton(
        "Post-process into bins of");
    ...
    public ATXYPlotSetupPanel(ATXYPlot panel, JFrame frame) {
      ...
      itsShowAllMatches
          .setToolTipText("Draw a point for every matched X and Y point");
      itsPostProcess.addActionListener(this);
      itsPostProcess.setActionCommand("post-proc");
      itsPostProcess
          .setToolTipText("Display a processed quantity, not just every X/Y match");
       ...
       * 
       * itsAxis.add(new AxisSetup()); itsAxisPanel.add((JPanel)itsAxis.get(0));
       */

      // add(new JScrollPane(itsSetupPanel,
    ...
    public void actionPerformed(ActionEvent e) {
      ...
      if (cmd.equals("Dynamic")) {
        ...
        System.err.println("ATXYPlotSetupPanel: got unhandled action: " + cmd);
      }
    }
    ...
    protected SavedSetup getSetup() {
      ...
      if (xpoints.size() == 0 || ypoints.size() == 0
        ...
        JOptionPane.showMessageDialog(this, "You need to select at least one\n"
            + "X and Y point, and there must be\n"
            + "a corresponding X point for every\n" + "Y point and vice versa",
            "Bad selections for X and Y points", JOptionPane.WARNING_MESSAGE);
        return null;
      }
      ...
      if (itsXAutoScale.isSelected()) {
        ...
        if (itsXAutoZero.isSelected()) {
          setup.put("Xshowzero", "true");
        } else {
          setup.put("Xshowzero", "false");
        }
      } else {
        setup.put("Xscalemode", "fixed");
        // Check that the numeric fields contain parsable number
        try {
          Double.parseDouble(itsXScaleMax.getText());
          Double.parseDouble(itsXScaleMin.getText());
        } catch (Exception e) {
          JOptionPane.showMessageDialog(this,
      ...
      if (itsYAutoScale.isSelected()) {
        ...
        if (itsYAutoZero.isSelected()) {
          setup.put("Yshowzero", "true");
        } else {
          setup.put("Yshowzero", "false");
        }
      } else {
        setup.put("Yscalemode", "fixed");
        // Check that the numeric fields contain parsable number
        try {
          Double.parseDouble(itsYScaleMax.getText());
          Double.parseDouble(itsYScaleMin.getText());
        } catch (Exception e) {
          JOptionPane.showMessageDialog(this,
      ...
      if (itsPostProcess.isSelected()) {
        ...
        try {
          Double.parseDouble(itsBinSize.getText());
        } catch (Exception e) {
          JOptionPane.showMessageDialog(this,
  ...
  public abstract class DataMatcher {
     ...
     */
    protected Number toNumber(Object data) {
      if (data == null) {
        return null;
      }
  ...
  /**
   * DataMatcher that finds the first Y point <i>after or coincident</i> with
   * the time-stamp of each X points. However it will not map two X points to
   * the same Y point, so if the same Y point matches two X's, the first X will
   * be discarded and only the second X will be matched.
   */
      ...
      for (int x = 0; x < xdata.size(); x++) {
        ...
        for (int y = lasty; y < ydata.size(); y++) {
          ...
          if (ytime.isAfterOrEquals(xtime)) {
            ...
            if (x + 1 == xdata.size()
                || ytime
                    .isBefore(((PointData) xdata.get(x + 1)).getTimestamp())) {
  ...
  /**
   * DataMatcher that will find the Y point that was sampled nearest to the time
   * that each X point was sampled. However each X and Y point can be used at
   * most once, so if the same Y point is the nearest Y point to two or more X
   * points, only the X point that is closest to the epoch of the y point will
   * actually be matched.
   */
        ...
        for (int y = lasty; y < ydata.size(); y++) {
          ...
          if (besty != thisy || y == ydata.size() - 1) {
            ...
            if (x + 1 == xdata.size()) {
              ...
              thisdiff = ((PointData) xdata.get(x + 1)).getTimestamp()
                  .getValue()
  ...
  /**
   * DataMatcher that will match all Y points to the nearest X point. This
   * differs from most of the other DataMatchers in that the others return only
   * the best match, not all matches.
   */
  ...
  /**
   * DataMatcher that finds the Y points that bound each X point and then
   * interpolates to find the Y value at the instant that each X point was
   * sampled.
   */
      ...
      for (int x = 0; x < xdata.size(); x++) {
        ...
        for (int y = lasty; y < ydata.size(); y++) {
          ...
          if (besty != y || y == ydata.size() - 1) {
            ...
            AbsTime yt2 = null;
            if (xtime.getValue()
                - ((PointData) ydata.get(besty)).getTimestamp().getValue() > 0) {
            ...
            if (yd1 != null && yd2 != null) {
              ...
              double Yval = yd1.doubleValue() + slope
                  * (xtime.getValue() / 1000000 - yt1.getValue() / 1000000);
              Number Xval = toNumber(thisx.getData());
              if (Xval != null) {
                res.add(new XYDataItem(Xval, new Double(Yval)));
              }
            }
            lasty = besty;
            break;
  ...
  /**
   * Indicates if post-processing should be performed after the X and Y points
   * have been matched.
   */
  ...
  public void run() {
    ...
    while (itsKeepRunning) {
      ...
      if (itsRealtime) {
        ...
        for (int i = 0; i < itsXPointNames.size(); i++) {
          ...
          while (x.size() > 0) {
            ...
            if (pd.getTimestamp().isAfter(itsStart)) {
              break;
            }
            x.removeElementAt(0);
          }
      ...
      System.err.println("Xpoints=" + itsXPointNames.size() + " Ypoints="
          + itsYPointNames.size() + " Series=" + itsData.getSeriesCount());
      for (int i = 0; i < itsXPointNames.size(); i++) {
        ...
        if (itsPostProcess) {
          if (itsShowAverages) {
            ...
            xy1 = getRMS(xy1);
          } else if (itsShowCounts) {
            xy1 = getCounts(xy1);
          }
        }
        XYSeries xy2 = itsData.getSeries(i);
        xy2.clear();
        System.err.println("ATXYPlot: series " + i + " has "
            + xy1.getItemCount() + " matches from " + xdata.size() + "/"
            + ydata.size());
  ...
  /** */
  public void actionPerformed(ActionEvent e) {
    System.err.println("ATXYPlot: Time to update graph \"" + itsTitle + "\"");
    synchronized (this) {
      this.notifyAll();
    }
  }
  ...
  public boolean loadSetupReal() {
    try {
      ...
      final JFreeChart chart = ChartFactory.createXYLineChart(itsTitle,
          itsXLabel, itsYLabel, itsData, PlotOrientation.VERTICAL,
      ...
      if (temp.equals("auto")) {
        ...
        System.err
            .println("ATXYPlot:loadData: Unknown X mode \"" + temp + "\"");
        blankSetup();
        return false;
      }
      ...
      if (temp.equals("auto")) {
        ...
        System.err
            .println("ATXYPlot:loadData: Unknown Y mode \"" + temp + "\"");
        blankSetup();
        return false;
      }
...
//          ((StandardXYItemRenderer) renderer).setPlotShapes(false);
      ...
      for (int i = 0; i < itsXPointNames.size(); i++) {
        ...
        Vector v = getArchive((String) itsXPointNames.get(i), itsStart, endtime);
        itsXData.add(v);
        v = getArchive((String) itsYPointNames.get(i), itsStart, endtime);
        itsYData.add(v);
      }
  ...
  /**
   * Bin the XY points according to <i>itsBinSize</i> and then return a new
   * XYSeries that contains the average Y value for each X bin.
   */
  protected XYSeries getAverages(XYSeries xypoints) {
    ...
    System.err.println("ATXYPlot: got " + numbins + " bins for xmin=" + xmin
        + " and xmax=" + xmax);
  ...
  /**
   * Bin the XY points according to <i>itsBinSize</i> and then return a new
   * XYSeries that contains the RMS Y value for each X bin.
   */
  protected XYSeries getRMS(XYSeries xypoints) {
    // Have to find the maximum and minimum X values
    int xylen = xypoints.getItemCount();
    double xmin = 0.0;
    double xmax = 0.0;
  ...
  /**
   * Bin the XY points according to <i>itsBinSize</i> and then return a new
   * XYSeries that contains the number of Y values in each X bin.
   */
  protected XYSeries getCounts(XYSeries xypoints) {
    // Have to find the maximum and minimum X values
    int xylen = xypoints.getItemCount();
    double xmin = 0.0;
    double xmax = 0.0;
  ...
  public static void main(String[] argv) {
     ...
     * BoxLayout.Y_AXIS));
     * 
     * ATXYPlot ts1 = new ATXYPlot(); SavedSetup s1 = new
     * SavedSetup("ant.temps", "atnf.atoms.mon.gui.monpanel.ATXYPlot", "Antenna
     * Pedestal
     * Temperatures:true:86400000000:0:2:ca05.ant.PEDTEM:ca06.ant.PEDTEM");
     * SavedSetup s2 = new SavedSetup("seemon.phases",
     * "atnf.atoms.mon.gui.monpanel.ATXYPlot", "Seeing Monitor
     * Phases:true:86400000000:0:1:seemon.site.seemon.Phase:"); SavedSetup s3 =
     * new SavedSetup("clock.tickphase", "atnf.atoms.mon.gui.monpanel.ATXYPlot",

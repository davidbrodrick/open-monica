...
{
  /** Class to keep track of currently open windows, offer a menu-list of
   * those windows to the user via each window's <i>Windows</i> menu, and
   * raise windows to the foreground when they are selected. */
  ...
  implements ActionListener
  {
    ...
    /** Called when a window is selected from the menu. The window which
     * was selected is raised to the screen foreground (hopefully). */
    public synchronized
    void
	...
	SavedSetup reqsetup = MonClientUtil.getSetup("atnf.atoms.mon.gui.MonFrame",
						     action);
	if (reqsetup!=null) {
     ...
     * @param i The index of the MonFrame to return.
     ...
     * @param frame The new Window to register.
     ...
     * @param frame The window to deregister. */
    public synchronized
    void
    remove(MonFrame frame)
    {
    ...
    /** Update the <i>Windows</i> menu in all windows. The updated menus
     * will list all currently open windows as options. */
    public synchronized
    void
    rebuildMenus()
    {
     ...
     * @param frame The frame to which the menu belongs. */
  ...
  /** Records number of MonFrames which have been created. This is just
   * used for coming up with default names for the window titles. */
  ...
  /** Last loaded setup, might be null if the setup has been modified
  ...
  MonFrame()
  {
    ...
    tempMenu.setAccelerator(KeyStroke.getKeyStroke(
    ...
    itsMainPanel.setLayout(new BoxLayout(itsMainPanel,
					 BoxLayout.Y_AXIS));
    //itsTabs.setTabPlacement(JTabbedPane.RIGHT);
   ...
   * @param newpan The panel to add to this frame. */
  protected
  void
  addPanel(MonPanel newpan)
  {
    itsSetup = null;
  ...
  /** Add a new MonPanel to this frame and redraw screen right away.
   * @param newpan The panel to add to this frame. */
  protected
  void
  addPanelNow(MonPanel newpan)
  {
    itsSetup = null;
    newpan.setFrame(this);
    newpan.setBorder(BorderFactory.createLoweredBevelBorder());
   ...
   * @param deadpan The MonPanel to remove from this frame.
  ...
  /** Configure this MonFrame to use the specified setup. This method can
   * be used to restore saved states, eg what MonPanels to show in the
   * window and what information they should each display.
   * @param setup Window setup information to use.
  ...
  /** Dump the current screen to a .PNG image file. Returns false if the
   * dump failed. */
  protected
  boolean
  dumpPNG(String filename)
  {
  ...
  public
  void
  setAutodump(int interval, String dumpfile)
  {
  ...
  /** Read a "saved state" file that describes what windows to create
   * at application start-up and how to arrange them. */
  public static
  boolean
  recoverArrangement(String filename)
  {
    try {
      //Record how many windows are currently open- we'll need to close them
  ...
  /** Save a description of what windows are open and how they are
   * arranged to the specified file name. */
  public static
  boolean
  saveArrangement(String filename)
  {
    try {

 ...
 * @see MonPanel
 */
public abstract class MonPanelSetupPanel extends JPanel implements ActionListener
{
    /**
     * The Frame we're displayed in. We need this reference for bringing up dialog boxes.
     */
    ...
    /**
     * The setup of the MonPanel before we started messing with it. This copy will be used
     * to "undo" and changes if the user hits the cancel button.
     */
    protected SavedSetup itsInitialSetup = null;

    /**
     * Records if we've altered the MonPanel's setup. If this is <tt>false</tt> then we
     * haven't touched the panel's setup, it should still be the same as the intial setup.
     */
    protected boolean itsChangedSetup = false;

    /**
     * C'tor, must be called by sub-classes.
     * @param monp The MonPanel we are going to configure. Must NOT be <tt>null</tt>.
     * @param frame The JFrame our panel will be displayed in.
     */
    protected MonPanelSetupPanel(MonPanel monp, JFrame frame)
    {
        super();
        itsMonPanel = monp;
        ...
        /*
         * JButton tempbut = new JButton("Peek"); tempbut.addActionListener(this);
         * tempbut.setActionCommand("Peek"); temppanel.add(tempbut);
         */

        /*
         * tempbut = new JButton("Apply"); tempbut.addActionListener(this);
         * tempbut.setActionCommand("Apply"); temppanel.add(tempbut);
         */
    ...
    /**
     * Called when buttons are pushed. Sub-classes should ensure that they call this super
     * method prior to performing their own handling.
     */
    ...
    /**
     * Return the current setup, as determined by the GUI controls. All sub-classes must
     * implement this method. It provides the means of extracting the setup specified by
     * the user into a useable format.
     * @return SavedSetup specified by GUI controls, or <tt>null</tt> if no setup can be
     * extracted from the GUI at present.
     */
    protected abstract SavedSetup getSetup();

    /**
     * Configure the GUI to display the given setup.
     * @param setup The setup to display to the user.
     */
    protected abstract void showSetup(SavedSetup setup);
}

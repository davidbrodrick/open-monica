 ...
 *
 * <P>In the first MonPanel implementation, each class had a routine for
 * serialising its current setup into a fixed format String. This was okay
 * but it was difficult to extend the fixed String format, eg, if we wanted
 * to add a new field.
 *
 * <P>The selected solution is to use a named-value format for storing MonPanel
 * setup information. However because we don't want to be locked-in to a
 * particular JVM/HashMap version we need to manage the serialisation
 * ourselves. This class is basically just a HashMap which has some home-grown
 * methods for serialising/deserialising to/from a String.
...
implements NamedObject, Comparable
{
  ...
  public
  SavedSetup(String name, String classname)
  {
    super();
    itsName = name;
    itsClass = classname;
  ...
  public
  SavedSetup(String name, String classname, int len)
  {
    super(len);
    itsName = name;
    itsClass = classname;
  ...
  toString()
  {
    ...
    while (it.hasNext()) {
      ...
      String temp = "`" + escape((String)me.getKey())
  ...
  /** Create an instance of the class this setup is for, using the

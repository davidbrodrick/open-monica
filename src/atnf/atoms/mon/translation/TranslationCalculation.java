 ...
 * of monitor points which will be listened to. The next N arguments must be the
 * names of the points being listened to. The final argument is the expression
 * to be evaluated where each input variable is named a, b, c, etc,
 * corresponding to the order they were named.
 * <P>
 * This implementation clears all inputs after a value is calculated, so that a
 * subsequent calculation will not happen until all inputs have updated again.
 * 
 * @author David Brodrick
 */
public class TranslationCalculation extends TranslationListener
{
  protected static String[] itsArgs = new String[] { "Calculation", "Listens to two other points", "NumPoints", "Integer",
      "MonitorPoint 1", "String", "MonitorPoint N", "String", "Function", "String" };
  ...
  /**
   * Provide the current input values to the expression parser, calculate result
   * and clear all input variables so that fresh values will be used next time.
   */

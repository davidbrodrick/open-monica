...
public abstract class PointArchiver extends Thread
{
     ...
     * idea is to prevent the server bogging down if there is a very large request.
    ...
    /**
     * Return the last update which precedes the specified time. We interpret 'precedes'
     * to mean data_time<=req_time.
     * @param pm Point to extract data for.
    ...
    /**
     * Archive the data for the given point. Note this actually places the data into a
     * write-out buffer, the data may not be flushed to disk immediately. The
     * <tt>saveNow</tt> does the real archiving.
    ...
    /**
     * Archive the Vector of data for the given point. Note this actually places the data
     * into a write-out buffer, the data may not be flushed to disk immediately. The
     * <tt>saveNow</tt> does the real archiving.
     * @param pm The point that the data belongs to

...
public class MonFrame extends JFrame implements ActionListener {
	...
	public static class WindowManager implements ActionListener {
			...
			if (action.equals("New Blank Window")) {
				...
				if (returnVal == JFileChooser.APPROVE_OPTION) {
					try {
						...
						if (!recoverArrangement(file.getAbsoluteFile()
								.toString())) {
							throw new Exception(
									"Unable to properly load the file");
						}
					} catch (Exception f) {
						// No Joy..
						f.printStackTrace();
				...
				if (returnVal == JFileChooser.APPROVE_OPTION) {
					try {
						...
						if (!saveArrangement(file.getAbsoluteFile().toString())) {
							throw new Exception(
									"Unable to properly save the file");
						}
					} catch (Exception f) {
						// No Joy..
						f.printStackTrace();
				...
				int num = Integer
						.parseInt(action.substring(action.indexOf(":") + 1));
				...
				if (selected != null) {
					...
					System.err
							.println("MonFrame:WindowManager: Can't remove Monframe");
				}
				...
				SavedSetup reqsetup = MonClientUtil.getSetup(
						"atnf.atoms.mon.gui.MonFrame", action);
				if (reqsetup != null) {
		...
		/**
		 * Add a new window to the registry. The <i>Windows</i> menu on all
		 * current windows will be updated to show the new window. A menu for
		 * display on the new window is returned.
		 * 
		 * @param frame
		...
		public synchronized void rebuildMenus() {
			...
			for (int i = 0; i < itsWindows.size(); i++) {
				rebuildMenu((JMenu) itsMenus.get(i),
						(MonFrame) itsWindows.get(i));
			}
		}
		...
		private synchronized void rebuildMenu(JMenu menu, MonFrame frame) {
			synchronized (menu) {
				...
				temp.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_N,
						ActionEvent.CTRL_MASK));
				temp.setMnemonic(KeyEvent.VK_N);
				tempmenu = new JMenu("New Window");
				tempmenu.add(temp);
				tempmenu.addSeparator();
				MonClientUtil.getSetupMenu(MonFrame.class.getName(), this,
						tempmenu);
				menu.add(tempmenu);
				menu.addSeparator();
				...
				temp.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_C,
						ActionEvent.CTRL_MASK));
				...
				temp.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_Q,
						ActionEvent.CTRL_MASK));
	...
	/**
	 * Records number of MonFrames which have been created. This is just used
	 * for coming up with default names for the window titles.
	 */
	...
	private Vector itsPanels = new Vector();

	/**
	 * Vector holds reference to the setup configuration panels for each
	 * MonPanel.
	 */
	...
	public MonFrame() {
		...
		exportMenu
				.setToolTipText("Export the information in a different format");
		exportMenu.setMnemonic(KeyEvent.VK_E);
		JMenuItem tempMenu = new JMenuItem("Print");
		tempMenu.addActionListener(this);
		tempMenu.setActionCommand("Print");
		tempMenu.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_P,
				ActionEvent.CTRL_MASK));
		...
		tempMenu.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_D,
				ActionEvent.CTRL_MASK));
		...
		tempMenu.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_E,
				ActionEvent.CTRL_MASK));
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (action.equals("Clear Frame")) {
			...
			SavedSetup reqsetup = MonClientUtil.getSetup(this.getClass()
					.getName(), action);
			if (reqsetup != null) {
				loadSetup(reqsetup);
			} else {
...
;
	...
	private void rebuildSubPanelMenu() {
		...
		if (itsPanels.size() > 0) {
			...
			for (int i = 0; i < itsPanels.size(); i++) {
				...
				tempMenu2.setToolTipText("Remove a " + desc
						+ " from the display");
	...
	public synchronized void rebuildDisplayMenu() {
		...
		MonClientUtil.getSetupMenu(this.getClass().getName(), this,
				itsDisplayMenu);
	}

	/**
	 * Configure this MonFrame to use the specified setup. This method can be
	 * used to restore saved states, eg what MonPanels to show in the window and
	 * what information they should each display.
	 * 
	 * @param setup
	...
	public boolean loadSetup(SavedSetup setup) {
		try {
			...
			if (!setup.checkClass(this)) {
				System.err.println("MonFrame:loadSetup: setup not for "
						+ this.getClass().getName());
				return false;
			}
			...
			for (int i = 0; i < numpanels; i++) {
				...
				try {
					...
					JOptionPane
							.showMessageDialog(
									this,
									"An exception occurred while creating an\n\""
											+ panelsetup.getClassName()
											+ "\"\n"
											+ "for the setup called \""
											+ panelsetup.getName()
			...
			for (int i = 0; i < numpanels; i++) {
				...
				if (!newpanel.loadSetup(newsetup)) {
					...
					// No need for a dialog as the panel will have already shown
					// one
					// But we do need to abort the loading of this setup
					System.err
							.println("MonFrame:loadSetup: Sub-panel failed to load setup - ABORTING");
					blankSetup();
					return false;
				}
				MonPanelSetupPanel sp = (MonPanelSetupPanel) itsSetupPanels
						.get(i);
				if (sp != null) {
			...
			e.printStackTrace();
			JOptionPane.showMessageDialog(this,
					"The setup \"" + setup.getName()
							+ "\"\ncould not be parsed.\n\n"
							+ "This Exception was encountered:\n"
							+ e.getClass().getName(), "Error Loading Setup",
					JOptionPane.WARNING_MESSAGE);
			blankSetup();
			return false;
		}
	...
	public static boolean recoverArrangement(String filename) {
		try {
			...
			while (f.ready()) {
				...
				SavedSetup setup = MonClientUtil.getSetup(
						"atnf.atoms.mon.gui.MonFrame", st.nextToken());
			...
			for (int i = 0; i < numalreadyopen; i++) {
				MonFrame frame = (MonFrame) theirWindowManager.itsWindows
						.get(0);
				theirWindowManager.actionPerformed(new ActionEvent(
						theirWindowManager, 0, "Close:" + frame.getNumber()));
	...
	private static void usage() {
		...
		System.err
				.println("-d\tDump screenshots to PNG images every \"interval\" seconds");
		System.err.println("-o\tDump the PNG images to this directory");
		System.err
				.println("-f\tLoad the following \"arrangement\" file at startup");
		System.err
				.println("setup\tName of setup(s) to be loaded, eg \"evironment.weather.summary\"\n");
		System.exit(1);
	}
	...
	public static void main(String[] argv) {
		...
		if (argv.length > 0) {
			for (int i = 0; i < argv.length; i++) {
				if (argv[i].equals("-h") || argv[i].equals("--help")) {
					...
					if (argv.length == i + 1) {
						System.err
								.print("ERROR: You must specify an interval in "
					...
					try {
						...
						System.err
								.println("ERROR: You must specify an interval in "
										+ "seconds for autodump mode!");
						System.err.println("\teg, " + argv[i] + " 60");
						System.exit(1);
					}
					...
					if (argv.length == i + 1) {
						System.err
								.print("ERROR: You must specify the target directory after"
										+ " the \"" + argv[i] + "\" argument");
						System.exit(1);
					}
					outputdir = argv[i + 1];
					...
					if (argv.length == i + 1) {
						System.err
								.print("ERROR: You must specify the file name after"
										+ " the \"" + argv[i] + "\" argument");
						System.exit(1);
					}
					if (!recoverArrangement(argv[i + 1])) {
						System.err.println("ERROR: The arrangement file \""
								+ argv[i + 1]
					...
					// page
					MonFrame frame = new MonFrame();
					SavedSetup def = MonClientUtil.getSetup(
							"atnf.atoms.mon.gui.MonFrame", argv[i]);
					if (def != null) {
						frame.loadSetup(def);
						frame.pack();
						numframes++;
						...
						System.err
								.println("\nERROR: You asked me to display setup \""
										+ argv[i] + "\" but it doesn't exist!");
						usage();
					}
				}
			}
		...
		if (numframes == 0) {
			...
			if (osname.indexOf("win") != -1) {
				monfile = "\\Application Data\\MoniCA\\default-arrangement-"
						+ MonClientUtil.getServerName() + ".txt";
			} else {
				monfile = "/.MoniCA/default-arrangement-"
						+ MonClientUtil.getServerName() + ".txt";
			}
			...
			if (!recoverArrangement(monfile)) {
				...
				SavedSetup def = MonClientUtil.getSetup(
						"atnf.atoms.mon.gui.MonFrame", "default");
				if (def != null) {
					frame.loadSetup(def);
				}
			}
			...
			if (autodumpperiod != 60000) {
				for (int i = 0; i < theirWindowManager.numWindows(); i++) {
					...
					framei.setAutodump(autodumpperiod, outputdir
							+ framei.getSetup().getName() + ".png");
				}
			}
		}
	}

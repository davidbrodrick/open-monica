...
{
    /**
     * Class to keep track of currently open windows, offer a menu-list of those windows
     * to the user via each window's <i>Windows</i> menu, and raise windows to the
     * foreground when they are selected.
     */
    public static class WindowManager implements ActionListener
    {
        ...
        /**
         * Called when a window is selected from the menu. The window which was selected
         * is raised to the screen foreground (hopefully).
         */
         ...
         * windows will be updated to show the new window. A menu for display on the new
         * window is returned.
        ...
        /**
         * Deregister the specified window. The window will be removed from the <i>Windows</i>
         * list of all remaining windows.
        ...
        /**
         * Update the <i>Windows</i> menu in all windows. The updated menus will list all
         * currently open windows as options.
         */
    ...
    /**
     * Records number of MonFrames which have been created. This is just used for coming
     * up with default names for the window titles.
     */
    ...
    /**
     * Last loaded setup, might be null if the setup has been modified since it was
     * initially loaded.
     */
     ...
     * @return <tt>true</tt> if removal went OK, <tt>false</tt> if the removal failed
     * (eg, maybe that panel doesn't exist within this frame).
     */
    ...
    /**
     * Configure this MonFrame to use the specified setup. This method can be used to
     * restore saved states, eg what MonPanels to show in the window and what information
     * they should each display.
    ...
    /**
     * Get the current configuration for this MonFrame. This can be used to capture the
     * current state of the MonFrame so that it can be easily recovered later.
    ...
    /**
     * Read a "saved state" file that describes what windows to create at application
     * start-up and how to arrange them.
     */
    ...
    /**
     * Save a description of what windows are open and how they are arranged to the
     * specified file name.
     */
        ...
        if (argv.length > 0) {
            for (int i = 0; i < argv.length; i++) {
                if (argv[i].equals("-h") || argv[i].equals("--help")) {
                    ...
                    if (argv.length == i + 1) {
                        System.err
                                        .print("ERROR: You must specify the target directory after" + " the \"" + argv[i]
                                                        + "\" argument");
                        System.exit(1);
                    }
                    outputdir = argv[i + 1];

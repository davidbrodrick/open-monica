...
/**
 * <i>AlarmCheck</i> sub-class which checks if a String data-value matches a predefined set. We can work in one of two ways, either
 * considering the new value to be <i>okay</i> if it matches one of the strings in the set, or considering it to be <i>in error</i>
 * if it matches a string in the set.
 * <P>
 * An optional argument may be provided first which is the integer number of update cycles before an alarm is flagged. The default
 * is 1.
 * <P>
 * The next, possibly first, first argument determines which of these behaviours we use. If the argument is <tt>true</tt> then we
 * consider a string match be be good. If the first argument is false then we consider a string match to be bad.
 * <P>
...
public class AlarmCheckStringMatch extends AlarmCheck {
  ...
  public AlarmCheckStringMatch(PointDescription parent, String[] args) throws IllegalArgumentException, NumberFormatException {
    super(parent, args);
    ...
    try {
      ...
      // If the first argument provided is an Integer value then user has specified minimum number of updates before raising an
      // alarm.
      try {

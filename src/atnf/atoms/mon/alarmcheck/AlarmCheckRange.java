...
/**
 * <i>AlarmCheck</i> sub-class which checks if a numeric data value is within a nominated range. The first two <i>init</i> arguments
 * should be the lower and upper limits of the range. An optional third argument defines whether inside the range is considered
 * nominal (<tt>True</tt> - the default) or outside the range is nominal (<tt>False</tt>). An optional fourth argument specifies the
 * number of update cycles that the value must be outside of the target range before the alarm state is actually flagged (default is
 * 1 update).
 * 
...
public class AlarmCheckRange extends AlarmCheck {
  ...
  /**
   * Is it normal to be inside the range (<tt>true</tt>) or is a normal value outside the range (<tt>false</tt>).
   */
  ...
  public AlarmCheckRange(PointDescription parent, String[] args) throws IllegalArgumentException {
    super(parent, args);
  ...
  public boolean checkAlarm(PointData data) {
    ...
    if (myData instanceof Number) {
      ...
      Logger logger = Logger.getLogger(Factory.class.getName());
      logger.error("AlarmCheckRange: ERROR: " + data.getName() + " has NON-NUMERIC data!");
      return true;
    }

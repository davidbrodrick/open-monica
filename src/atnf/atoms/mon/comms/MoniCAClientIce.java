...
public class MoniCAClientIce extends MoniCAClient {
  ...
  /**
   * Add multiple points to the servers. This is a privileged operation which requires the
   * user to authenticate against the server. The username and password are encrypted
   * prior to transmission over the network.
  ...
  public Vector<Vector<PointData>> getArchiveData(Vector<String> pointnames, AbsTime start, AbsTime end, int maxsamples)
    ...
    try {
      ...
      for (int thispoint = 0; thispoint < pointnames.size(); thispoint++) {
        ...
        while (true) {
          PointDataIce[][] icedata = itsIceClient.getArchiveData(new String[] { pointnames.get(thispoint) }, thisstart.getValue(), end.getValue(), maxsamples);
          if (icedata != null && icedata.length > 0 && icedata[0].length > 0) {
            // Convert data to native representation
            //System.err.println("MoniCAClientIce.getArchiveData: " + pointnames.get(thispoint) + ": " + icedata[0].length + " New Data");
  ...
  /**
   * Set new values for the specified points. This requires authentication. The username
  ...
  /**
   * Return all SavedSetups for client Objects from the server. <tt>null</tt> may be
   * returned if the server has no SavedSetups.
   */
   ...
   * inappropriate modification of the server data. The username and password are
   * encrypted prior to transmission over the network.
  ...
  /**
   * Return an RSA encryptor that uses the servers public key and modulus. This will allow
   * us to encrypt information that can only be encrypted by the server.
   */

...
public class ModbusInterface extends ExternalSystem
{
   ...
   * INPUT:	uid = unit ID 
   * 		ref = starting reference
   *		count = number of coils from starting reference to read
   */
  public synchronized ReadCoilsResponse readCoils(int uid, int ref, int count) throws Exception {
   ...
   /**
   * Read discrete inputs. (Modbus function 0x02)
   * INPUT:	uid = unit ID
   * 		ref = starting reference 
 ...
 /**
   * Write multiple registers. (Modbus function 0x10)
   * INPUT:	uid = unit ID
   * 		ref = starting reference
   *		registers
   */
		...
		try{
			switch (FCode) {
				case 1:
					...
					if(!useArray) pm.firePointEvent(new PointEvent(this, new PointData(pm.getFullName(), new Boolean(rc_response.getCoilStatus(0))), true));
					else{
						Boolean[] rc_Array = new Boolean[numberToRead];
						for(int j=0; j < (numberToRead); j++) rc_Array[j] = new Boolean(rc_response.getCoilStatus(j));
				...
				case 2:
					...
					if(!useArray) pm.firePointEvent(new PointEvent(this, new PointData(pm.getFullName(), new Boolean(di_response.getDiscreteStatus(0))), true));
					else{
						Boolean[] di_Array = new Boolean[numberToRead];
						for(int j=0; j < (numberToRead); j++) di_Array[j] = new Boolean(di_response.getDiscreteStatus(j));
				...
				case 3:
					...
					if(!useArray) pm.firePointEvent(new PointEvent(this, new PointData(pm.getFullName(), new Integer(rhr_response.getRegisterValue(0))), true));
						...
						for(int j=0; j < (numberToRead); j++) rhr_Array[j] = new Integer(rhr_response.getRegisterValue(j));
				...
				case 4:
					...
					if(!useArray) pm.firePointEvent(new PointEvent(this, new PointData(pm.getFullName(), new Integer(rir_response.getRegisterValue(0))), true));
					else{
						Integer[] rir_Array = new Integer[numberToRead];
						for(int j=0; j < (numberToRead); j++) rir_Array[j] = new Integer(rir_response.getRegisterValue(j));

...
class ThyconUPS extends ASCIISocket {
  ...
  private void parseResponse(String resp, HashMap<String, Object> map) {
    ...
    if (bytes[3] == '8' && bytes[4] == '0') {
       ...
       * toFloat(bytes[5], bytes[6], bytes[7], bytes[8]) + "\t" + toFloat(bytes[9],
       * bytes[10], bytes[11], bytes[12]) + "\t" + toFloat(bytes[13], bytes[14],
       * bytes[15], bytes[16]));
       */
      ...
      // EE == Hex string representation of decimal alarm code. eg "48" == 72 == "DC AT
      // FLOAT"
      // TT == Hex string representation of decimal type code. eg "0A" == 10 == "NORM"
      ...
      if (msg == null)
        ...
        if (errNum == 64) {
          ...
          // Parameter == xxXX, where xx == Low order byte, XX == High order byte
          int mins = (BCD2byte(bytes[23]) << 4) + BCD2byte(bytes[24]) + (BCD2byte(bytes[25]) << 12) + (BCD2byte(bytes[26]) << 8);
          map.put("ALRMMSG", msg + " " + mins + " mins");
        } else if (errNum == 175) {
          // Parameter == xxXX, where xx == Low order byte, XX == High order byte
          int hours = (BCD2byte(bytes[23]) << 4) + BCD2byte(bytes[24]) + (BCD2byte(bytes[25]) << 12) + (BCD2byte(bytes[26]) << 8);

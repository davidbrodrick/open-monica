 ...
 * 
 * <p>
 * The value of dUTC changes from time to time. This class encapsulates those changes into an authorative class that will present
 * dUTC for any time in the past or into the near future.
 * </p>
 * 
 * <p>
 * The current value of dUTC is obtained from the ATOMS observatory property <code>dUTC</code>.
 * </p>
 * 
 ...
 */
final public class DUTC {
  /** MJD of 1970/01/01 */
  final static double MJD1970 = 40587.0;
   ...
   * 
   * @return The current dUTC (in seconds).
   */
   ...
   * @return The current dUTC (in seconds).
   */
  public static int get(double utc) {
    int n = leap.length;
    for (int i = 0; i < n; i++) {
      if (utc >= leap[i][0]) {
        return leap[i][1];
   ...
   * @return The current dUTC (in seconds).
   */
  public static int get(long bat) {
  ...
  /**
   * Get the list of all dUTC changes
   */
  public static SortedMap<Date, Integer> getLeapSeconds() {
    TreeMap<Date, Integer> res= new TreeMap<Date, Integer>();
    for (int i = 0; i < leap.length; i++) {
      long millisepoch = (long)(leap[i][0] - MJD1970)*86400000l;
      Date d = new Date(millisepoch);
      res.put(d, new Integer(leap[i][1]));
    }
    return res;
  }
  ...
  public static void main(String[] argv) {
    ...
    System.out.println("DUTC at 4000000000000000L (BAT 1985) = " + DUTC.get(4000000000000000L));
    System.out.println("DUTC at 3506716800000000L (BAT 1970) = " + DUTC.get(3506716800000000L));
  }

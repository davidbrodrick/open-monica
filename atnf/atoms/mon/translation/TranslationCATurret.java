 ...
 * @version $Id: TranslationCATurret.java,v 1.3 2008/01/30 04:53:30 bro764 Exp $
 */
public class
TranslationCATurret
extends Translation
{
  ...
  public
  TranslationCATurret(PointMonitor parent, String[] init)
  {
    super(parent, init);
  ...
  translate(PointData data)
  {
    ...
    String state;
    if (itsLSstate>0) state = "20/13cm";
    else if (itsCXstate>0) state = "6/3cm";
    else if (itsKQstate>0) {
      //Ensure we have translator data
      if (itsTranslatorState==-1) return null;
      //Use translator to determine on-axis feed
      if (itsTranslatorState==0) state = "12mm";
      else if (itsTranslatorState==1) state = "7mm";
      else state="off-axis";
    } else if (itsWFstate>0) state = "3mm";
    else state="off-axis";
